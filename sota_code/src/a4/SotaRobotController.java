...
public class SotaRobotController {
    ...
    private final Short[] NEUTRAL_POSE = {0, -725, -365, 725, 365, -160, -130, 0};
    
    // Nod pose: pitch head down then back to neutral
    private final Short[] NOD_POSE = {0, -725, -365, 725, 365, -160, 150, 0}; 
    private final int NOD_DURATION_MS = 200;
    
    // Tilt head (listening posture) - roll the head
    private final Short[] TILT_POSE = {0, -725, -365, 725, 365, -160, -60, 344};  // Head roll
    
    // Lower head (break posture) - look down
    private final Short[] LOWER_HEAD_POSE = {0, -700, -360, 700, 360, 0, 150, 0};  // Look down more
    ...
    private final Short[] POINT_LEFT_POSE = {800, -5, -100, 725, 365, -577, -130, 0};

    private final Short[] POINT_AT_USER_POSE = {0, -725, -365, 65, 290, -160, -130, 0};
    ...
    public SotaRobotController(Blackboard blackboard) {
        this.blackboard = blackboard;
        connect();
        piper = new PiperTTS("http://10.186.226.230:5000");// ============= Add your piper server address here =============
        voiceDetector = new VoiceKeywordDetector(blackboard);
    }
    ...
    public void speak(String text) {
        ...
        if (piper != null) {
            piper.speak(text);
            blackboard.lastKeyword = ""; // Clear keyword after speaking to prevent triggers from TTS and not user voice - starting and stopping the listening thread can cause a segfault
        }
    }
    ...
    public void setLEDColor(int r, int g, int b) {
        ...
        if (motion != null) {
            ...
            motion.play(nextPose, 30);
        }
    }
    


    public void pointAtUser() {
        System.out.println("[SOTA GESTURE] Point at user");
        
        playPoseAndWait(POINT_AT_USER_POSE, 400);
        waitMs(300);
    ...
    public void pointAtScreen() {
        System.out.println("[SOTA GESTURE] Point at the screen");
        playPoseAndWait(POINT_LEFT_POSE, 600);
        waitMs(500);
    ...
    public void nod() {
        ...
        playPoseAndWait(NOD_POSE, NOD_DURATION_MS);
