...
class GazeTracker:
    """
    A wrapper class for the L2CS-Net gaze estimation pipeline.
    """
    def __init__(self, model_dir="./l2cs_models", device=None):
        """
        Initializes the L2CS-Net pipeline.
        Downloads the pre-trained model automatically on first use.
        """
        ...
        self.gaze_pipeline = Pipeline(
            ...
            arch='ResNet50',  # Using ResNet50 backbone for good accuracy/speed balance
    ...
    def _download_model(self):
        """Downloads the pre-trained L2CSNet_gaze360.pkl model."""
        print("Downloading L2CS-Net pre-trained model (L2CSNet_gaze360.pkl)...")
        # You can use gdown or wget. The exact URL can be found on the repo's release page.
        # A common working URL is below. If it fails, please check the official repo for the latest link.
    ...
    def process_frame(self, frame):
        ...
        # Extract gaze angles (assuming results.gaze[0] gives yaw, pitch for the first face)
        gaze_data = None
        if results and results.yaw is not None and results.pitch is not None:
            ...
            try:
                ...
                # Fallback to our own drawing if the render function fails
    ...
    def get_gaze(self):
        """Return the latest gaze angles."""
        return self.last_gaze
    


    def calibrate_center(self, duration=3):
        """Run center calibration"""
        return self.interpreter.calibrate_center(self, duration)
    
    def is_looking_at_screen(self):
        return self.interpreter.is_looking_at_screen()
    ...
    def get_gaze_direction(self):
        return self.interpreter.get_gaze_direction()
